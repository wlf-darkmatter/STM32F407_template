...
/**
  ...
  * @param  USARTx: where x can be 1, 2, 3, 4, 5, 6, 7 or 8 to select the USART or 
  *         UART peripheral.本质是USART或UART的外设地址，其形式是一个数据结构，指向USARTx的base地址，
  *         【USART_TypeDef】该数据结构本身定义了USART的7个寄存器
  * @param  USART_InitStruct: 指向一个 【USART_InitTypeDef】类型的数据结构，是面向对象的。
  *         这个数据结构里包含了用户需要的对USART串口作出的初始化定义参数，如：波特率、字长、停止位等
  * @retval None
  */
...
/**
  ...
  * @note   该函数必须在 USART_Init() 之前启用，以保证波特率（baudrate）分频值的正确
  * 
  * @param  USARTx: where x can be 1, 2, 3, 4, 5, 6, 7 or 8 to select the USART or 
  *         UART peripheral.
...
uint16_t USART_ReceiveData(USART_TypeDef* USARTx)
{
  ...
  /* Receive Data */
  return (uint16_t)(USARTx->DR & (uint16_t)0x01FF);//传出DR[8:0]共9个比特，因为比特9或比特8有可能是奇偶校验位
}
...
/**
  * @brief  Enables or disables the specified USART interrupts.按照CR2，CR1，CR3的顺序置位
  * @param  USARTx: where x can be 1, 2, 3, 4, 5, 6, 7 or 8 to select the USART or 
  *         UART peripheral.
...
void USART_ITConfig(USART_TypeDef* USARTx, uint16_t USART_IT, FunctionalState NewState)
{
  ...
  /* The CTS interrupt is not available for UART4 and UART5 */
    ...
    /* Get the USART register index， 获取[7:5]位记录了该中断控制位处在哪个寄存器 */
    usartreg = (((uint8_t)USART_IT) >> 0x05);

    /* Get the interrupt position */
    //获取需要配置的中断位的位置，itpos==USART_IT[4:0]
...
/**
  * @brief  获取USART标志寄存器的特定位的状态
  * @param  USARTx: where x can be 1, 2, 3, 4, 5, 6, 7 or 8 to select the USART or 
  *         UART peripheral.
  * @param  USART_FLAG: 需要查看的状态位名称
  *          This parameter can be one of the following values:
  *            @arg USART_FLAG_CTS:  CTS标志位				CTS Change flag (not available for UART4 and UART5)
...
/**
  * @brief  获取USART的几个和中断有关的寄存器的信息，将分别访问CR1,CR2,CR3以及flag。
  * @param  USARTx: where x can be 1, 2, 3, 4, 5, 6, 7 or 8 to select the USART or 
  *         UART peripheral.
