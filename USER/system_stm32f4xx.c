/**
  ...
  *
  * @note	该版本已被修改，只单独适配STM32F407
  *
  * 1.  This file provides two functions and one global variable to be called from 
  *     user application:
  *      - SystemInit(): 初始化系统时钟 (System clock源, PLL倍增器 and 分频系数, 
  ...
  *        APB2 Prescaler                         | 2
  *-----------------------------------------------------------------------------
  *        HSE Frequency	                      | 25000000Hz=25MHz—修改为8MHz(其实无法修改，是自己给的系统认定的一个值)
  *-----------------------------------------------------------------------------
  *        PLL_M                                  | 25——修改为8，在RCC的一个寄存器中作为分频参数
  *-----------------------------------------------------------------------------
  *        PLL_N                                  | 336
  *-----------------------------------------------------------------------------
  *        PLL_P                                  | 2
...
#define PLL_P      2
//PLL的倍频系数在之后由stm32f4xx_rcc.c函数设置其倍频、分频的寄存器
...
void SystemInit(void)
{
  ...
  /* Set HSION bit */
  /*第一行代码操作时钟控制寄存器，将内部16MHz高速时钟（HSI）使能，从这里可以看出系统启动后是首先依靠内部时钟源而工作的。*/
  ...
  /*复位【HSE】的ON位，【CSS】的ON位和【PLL】的ON位*/
  RCC->CR &= (uint32_t)0xFEF6FFFF;//重设了CR[24,19,16]位为0

  /* Reset PLLCFGR register */
  RCC->PLLCFGR = 0x24003010;

  /* Reset HSEBYP bit */
  RCC->CR &= (uint32_t)0xFFFBFFFF;//重设了CR[18]位为0
  ...
  /* Configure the System clock source, PLL Multiplier and Divider factors, 
     AHB/APBx prescalers and Flash settings ----------------------------------*/
  /*配置 System Clock 的时钟源（HSI,HSE,PLL）、PLL的倍频和分频因子、APB和AHBx的预分频系数、FLASH的设置，时序啦，延时周期和使能预取缓冲期......*/
  SetSysClock();
...
/**
  ...
  *         
  *			(*) HSI_VALUE 是一个固定的值，该值在stm32f4xx.h文件中定义(默认值是16MHz)，
  *				但是该值随着电压和温度的变化会发生浮动。
  *    
  *         (**) HSE_VALUE 是一个固定的值，该值在stm32f4xx.h文件中定义(默认值是25MHz，已被修改为8MHz)
  *				 用户需要自己确定这个值是否和单片机上的晶振体的频率一致，否则，系统给予
  *              的时钟就是错的。
  *                
...
void SystemCoreClockUpdate(void)
{
  ...
  /* Get SYSCLK source -------------------------------------------------------*/
  tmp = RCC->CFGR & RCC_CFGR_SWS;//【系统时钟切换状态】寄存器
