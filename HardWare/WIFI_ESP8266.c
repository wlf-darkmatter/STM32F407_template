...
extern u8 WiFi_State;//判断是否处于Debug状态的8位数
//⑦	编写中断处理函数——函数格式应该为 USARTxIRQHandler（x为串口号）
extern u8 USART1_Busy;
void USART2_IRQHandler(void) { //串口2中断服务程序，【*】该函数的调用位置在启动文件startup_stm32f40_41xxx.s中
...
/*		if (WiFi_State & 0x80) {
		...
		if ((USART2_RX_STA & 0x8000) == 0) {//USART_RX_STA[15]==0  接收未完成
			if (USART2_RX_STA < USART2_REC_LEN)	{//还可以接收数据
				...
				if (USART2_RX_STA == 0)	TIM_Cmd(TIM7, ENABLE);//使能定时器7的中断 
