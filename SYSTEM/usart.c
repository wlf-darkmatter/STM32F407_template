...
int fputc(int ch, FILE *f){ 	
	...
	while((USART1_TX_STA&0x80)==0);//判断[7],循环发送,直到发送完毕
	USART1_TX_STA &= ~(0x80);//[7]清零
	/*
	##注意
	如果打开了TCIE的话，那么在
...
void USART1_IRQHandler(void) //串口1中断服务程序，【*】该函数的调用位置在启动文件startup_stm32f40_41xxx.s中
{
	...
	{	//USART_SR 比特5 RXNE位是【读取数据寄存器】不为空标志符，当RDR移位寄存器的内容已经传输到USART_DR寄存器时，该位硬件置1（SET）
		...
		if ((USART1_RX_STA & 0x8000) == 0) {//USART_RX_STA[15]==0  接收未完成
			if (USART1_RX_STA & 0x4000) {//USART_RX_STA[14]位是1，说明上次已经接收到了0x0D	
				if (Res != 0x0a)//查看这次是不是接收到了0x0A
					USART1_RX_STA = 0;//接收错误,重新开始
				else
					USART1_RX_STA |= 0x8000;	//这次接收到了0x0A，全部的接收完成了，USART_RX_STA[15]位置1 
			}
			else { //上次还没收到0x0D

				if (Res == 0x0d)//那就看看这次是不是0x0D
					USART1_RX_STA |= 0x4000;//[14]位置位，这次确实接收到了0x0D
				else {//这次不是0x0D，说明这次接收到的是一般的信息，要做记录
					USART1_RX_BUF[USART1_RX_STA & 0X3FFF] = Res;//0x3FFF - [13:0]，取后14位，表示已存数据的数量
					USART1_RX_STA++;//数据量加一
					if (USART1_RX_STA > (USART1_REC_LEN - 1))
						USART1_RX_STA = 0;//接收数据错误（超过200字节）重新开始接收
	...
	if (USART_GetITStatus(USART1, USART_IT_TC) != RESET) {
		USART_ClearITPendingBit(USART1, USART_IT_TC);//利用重定义的printf()时，不想重复进入TC中断死循环
		USART1_TX_STA |= 0x80;//置位[7]，说明进入了中断，并且是TC中断。
	}
